...
public class Student { // 클래스 선언부
 ...
 //    public        x    class Student
	...
	//    "필드 == 필드변수 == 멤버 변수" 내에 "인스턴스 변수"와 "클래스 변수"가 있다.
	...
	/* 필드 구성 (대괄호는 생략가능 하다는 의미)
	 * 
	 * 아래 표는 가로로 연결해서 생각하지 말것. 
	 * 각각 따로이니 세로로만 볼 것.
	 * 
	 ...
	 *   2) 반환형이 존재하지 않는다. (void, String등이 존재하지 않음)
	 * 
	 * - 생성자 종류
	 *   1) 기본 생성자
	 *   2) 매개변수 생성
	...
	// [접근제한자] 클래스명() { 코드 }
	public Student() {
		// 객체가 생성될 때(마다) 수행할 코드. (비워 둘 수도 있음)
		System.out.println("기본 생성자에 의해서 Student 객체가 생성됨.");
	}
	//그동안은 컴파일러(JVM)이 코드가 비어있는 기본생성자를 자동생성해준 것임.
