...
public class ClsService /*extends Student*/{
	...
	public void ex1() {
		...
		// public class인 Student는 import가 가능하다.
		
		//TestVO test = new TestVO();
		// (default) class인 TestVO는 import 불가(다른패키지라서)
...
//		//System.out.println( v3 ); // default -> x
//		//System.out.println( v4 ); // private -> x
	...
	public void ex2() {
		...
		// 학생 객체 2개 생성
		Student std1 = new Student(); //기본생성자 1
		Student std2 = new Student(); //기본생성자 2
		 ...
		 * (ex : main메서드 : 프로그램이 시작되고 나면 종료될때까지 사라지지 않음.)
		 * 
		 * 2) 사용 방법 : 클래스명.변수명
		 * 
		 */
	...
	public void ex3() {
		...
		// Student 객체를 기본 생성자를 이용해 생성하고 
		// 이를 참조하는 참조변수s1에 대입.
		// (s1 자료형은 Student)
		// (Student라는 변수는 Heap 영역에 생성)
		...
		//			왜? 같은 기본 생성자로user 객체로 생성했기 때문에
		
		System.out.println("----------------------------------");
		// 해결 방법 1: setter를 이용해서 새로운 값 대입
		
		u2.setUserId("abcd123456"); //abcd123456 : 매개변수
		u2.setUserPw("1a2b3c");
		u2.setUserName("김영희");
		u2.setUserAge(19);
		u2.setUserGender('여');
		...
		System.out.println(u3.getUserId() ); //VO에 this 안하면 null //test3
		System.out.println(u3.getUserPw() ); //VO에 this 안하면 null //password3
	...
	public void ex4() {
		...
		User u3 = new User ("user03", "pass03", "김용기", 30, '남'); // 매개변수 5개 생성
